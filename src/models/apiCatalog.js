...
export const apiDrama = async(setDramaMovies) => {
  try{
    let url = "https://api.themoviedb.org/3/discover/movie?api_key=478482cb8ce7c6d6fa5ecb5d066f3fff&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=18&with_watch_monetization_types=flatrate"
    const response = await fetch(url);
    const data = await response.json();
